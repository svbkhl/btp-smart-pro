import { Quote } from "@/hooks/useQuotes";

export const FAKE_QUOTES: Quote[] = [
  {
    id: "fake-quote-1",
    user_id: "fake-user",
    client_name: "M. Martin",
    project_id: "fake-proj-1",
    quote_number: "DEVIS-2024-001",
    status: "sent",
    estimated_cost: 45000,
    mode: "detailed",
    tva_rate: 20,
    subtotal_ht: 37500,
    total_tva: 7500,
    total_ttc: 45000,
    currency: "EUR",
    details: {
      estimatedCost: 45000,
      items: [
        { name: "Maçonnerie", quantity: 1, unitPrice: 25000 },
        { name: "Plomberie", quantity: 1, unitPrice: 12000 },
        { name: "Électricité", quantity: 1, unitPrice: 8000 },
      ],
    },
    sent_at: new Date(Date.now() - 25 * 24 * 60 * 60 * 1000).toISOString(),
    created_at: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 25 * 24 * 60 * 60 * 1000).toISOString(),
  },
  {
    id: "fake-quote-2",
    user_id: "fake-user",
    client_name: "Mme. Dupont",
    project_id: "fake-proj-2",
    quote_number: "DEVIS-2024-002",
    status: "draft",
    estimated_cost: 18000,
    mode: "simple",
    tva_rate: 20,
    subtotal_ht: 15000,
    total_tva: 3000,
    total_ttc: 18000,
    currency: "EUR",
    details: {
      estimatedCost: 18000,
      items: [
        { name: "Extension garage", quantity: 1, unitPrice: 18000 },
      ],
    },
    created_at: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
  },
  {
    id: "fake-quote-3",
    user_id: "fake-user",
    client_name: "Entreprise Bernard",
    project_id: "fake-proj-3",
    quote_number: "DEVIS-2024-003",
    status: "accepted",
    estimated_cost: 8500,
    mode: "detailed",
    tva_rate: 20,
    subtotal_ht: 7083.33,
    total_tva: 1416.67,
    total_ttc: 8500,
    currency: "EUR",
    payment_status: "pending",
    details: {
      estimatedCost: 8500,
      items: [
        { name: "Peinture intérieure", quantity: 120, unitPrice: 70 },
      ],
    },
    created_at: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 85 * 24 * 60 * 60 * 1000).toISOString(),
  },
  {
    id: "fake-quote-4",
    user_id: "fake-user",
    client_name: "M. et Mme. Leroy",
    project_id: "fake-proj-4",
    quote_number: "DEVIS-2024-004",
    status: "signed",
    estimated_cost: 65000,
    mode: "detailed",
    tva_rate: 20,
    subtotal_ht: 54166.67,
    total_tva: 10833.33,
    total_ttc: 65000,
    currency: "EUR",
    payment_status: "partially_paid",
    signed: true,
    signed_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
    signer_name: "M. Leroy",
    details: {
      estimatedCost: 65000,
      items: [
        { name: "Construction terrasse", quantity: 1, unitPrice: 25000 },
        { name: "Véranda", quantity: 1, unitPrice: 30000 },
        { name: "Aménagement jardin", quantity: 1, unitPrice: 10000 },
      ],
    },
    sent_at: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(),
    created_at: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
  },
  {
    id: "fake-quote-5",
    user_id: "fake-user",
    client_name: "SARL Construction Moderne",
    project_id: "fake-proj-5",
    quote_number: "DEVIS-2024-005",
    status: "rejected",
    estimated_cost: 125000,
    mode: "detailed",
    tva_rate: 20,
    subtotal_ht: 104166.67,
    total_tva: 20833.33,
    total_ttc: 125000,
    currency: "EUR",
    details: {
      estimatedCost: 125000,
      items: [
        { name: "Rénovation complète", quantity: 1, unitPrice: 125000 },
      ],
    },
    sent_at: new Date(Date.now() - 45 * 24 * 60 * 60 * 1000).toISOString(),
    created_at: new Date(Date.now() - 50 * 24 * 60 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 40 * 24 * 60 * 60 * 1000).toISOString(),
  },
];


