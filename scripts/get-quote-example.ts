 ...
 * 
 * @param lastId - ID du dernier devis récupéré (pour la pagination)
 * @param limit - Nombre de devis à récupérer (défaut: 10)
 * @param userId - ID de l'utilisateur (optionnel, pour filtrer)
 */
async function getQuotesCursor(lastId: string | null = null, limit: number = 10, userId?: string) {
 ...
 */
async function getAllQuotesForUser(userId: string, batchSize: number = 50) {
...
async function runExamples() {
  ...
  if (SUPABASE_ANON_KEY && SUPABASE_ANON_KEY !== 'YOUR_PUBLIC_ANON_KEY') {
    ...
    console.log('   Exemple: VITE_SUPABASE_ANON_KEY=your_key npx tsx scripts/get-quote-example.ts');
  }
}
...
export { getQuote, getQuotesCursor, getAllQuotesForUser };
