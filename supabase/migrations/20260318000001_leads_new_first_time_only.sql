-- Leads "Nouveaux" = uniquement la première fois (jamais encore modifiés par le closer).
-- Dès qu'un lead est mis à jour (statut ou notes), il passe en "assigné" (plus dans Nouveaux).

-- 1) Colonne first_seen_at : NULL = encore "nouveau", non NULL = déjà vu/traité
...
-- 2) Trigger : à la première mise à jour du lead (statut, notes, etc.), marquer comme "vu" (plus "nouveau")
...
-- 3) get_my_lead_stats : "new" = leads assignés à moi et jamais encore "vus" (first_seen_at IS NULL)
...
-- 4) get_closer_activity : "new" = first_seen_at IS NULL (cohérence avec get_my_lead_stats)
