...
CREATE TYPE public.app_role AS ENUM ('dirigeant', 'salarie', 'client');
...
-- Function sécurisée pour vérifier les rôles (évite les problèmes de récursion RLS)
...
CREATE POLICY "Dirigeants can view all roles"
  ON public.user_roles
  FOR SELECT
  USING (public.has_role(auth.uid(), 'dirigeant'));
...
CREATE TABLE public.notifications (
  ...
  related_table TEXT, -- 'maintenance_reminders', 'ai_quotes', etc.
  related_id UUID,
...
BEGIN
  -- Pour chaque dirigeant, créer une notification
  FOR dirigeant_record IN 
    SELECT user_id 
    FROM public.user_roles 
    WHERE role = 'dirigeant'
  LOOP
