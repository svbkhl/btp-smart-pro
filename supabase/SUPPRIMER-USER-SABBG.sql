...
-- ============================================
-- ⚠️ Ce script supprime l'utilisateur avec l'email sabbg.du73100.com
-- ============================================
...
BEGIN
  ...
  FROM auth.users
  WHERE email ILIKE '%sabbg.du73100.com%';
  
  IF v_user_id IS NULL THEN
    RAISE NOTICE '❌ Aucun utilisateur trouvé avec l''email contenant "sabbg.du73100.com"';
    RETURN;
  END IF;
  ...
  WHERE user_id = v_user_id
     OR identity_data->>'email' ILIKE '%sabbg.du73100.com%';
...
-- DELETE FROM auth.identities
-- WHERE identity_data->>'email' ILIKE '%sabbg.du73100.com%'
--    OR user_id IN (
--      SELECT id FROM auth.users WHERE email ILIKE '%sabbg.du73100.com%'
--    );
